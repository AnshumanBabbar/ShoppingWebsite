...
{
    public partial class ViewAllDenims : System.Web.UI.Page
    {
        ...
        private void BindProducts()
        {
            ...
            sql = "  select A.*,B.*,D.*,C.Name as BrandName, B.Name as ImageName,A.PPrice-A.PselPrice as DiscountAmount from tblProducts A inner join tblBrands C on C.BrandId = A.PBrandId inner join tblSubCategory D on A.PSubCatId = D.SubCatId cross apply(select top 1 * from tblProductImages where Pid = A.PId order by Pid desc)B where D.SubCatName = 'Denims' order by A.PId desc";
